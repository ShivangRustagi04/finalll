...
def show_summary():
    selected_article = article_combobox.get()
    summary_text.delete(1.0, tk.END)  # Clear the text widget
    summary_text.insert(tk.END, articles_and_summaries.get(selected_article, "Summary not available."))

# Create the main application window
root = tk.Tk()
root.title("Indian Constitution Articles")

# Create a Combobox to select articles
article_combobox = ttk.Combobox(root, width= 70, values=list(articles_and_summaries.keys()))
article_combobox.set("Select an Article")
article_combobox.pack()

# Create a Text widget to display the summary
summary_text = tk.Text(root, height=10, width=70)
summary_text.pack()

# Create a button to show the summary
show_button = tk.Button(root, text="Show Summary", command=show_summary)
show_button.pack()
